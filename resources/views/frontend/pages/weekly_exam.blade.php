         ...
         <div class="row">
            ...
            <div class="col-xl-6 col-md-6 col-sm-12" id="exam-start" style="display: none;">
		           <div class="mid_content">
		              <h1 class="pagetitle">Weekly Exam</h1>

		             
		              <div class="examtimecol">
		                 <div class="totaltime">Remaining Time - <span class="countdown"> Min</span></div>
		                 <div class="timecount">Total Time - <span class="counter" id="count">{{$questionSet->total_time}}</span> Min</div>  
		              </div>
            ...
            <div class="col-xl-6 col-md-6 col-sm-12" id="start-div">
               <div class="mid_content">
                  ...
                  <div class="content_box">  
                     ...
                     <div class="exammessagebox">
                        <p>The Exam will be start :</p>
                        <p>Exam Date:  {{date('d/m/Y', strtotime($questionSet->start_date))}} </p>
                        <p>Time: {{date('h : i A', strtotime($questionSet->time))}}</p> 
                     </div>
  ...
  $.ajax({
    ...
    $('#submitBtn').click(function(){
		...
		var submit_answer = $('input[name="answer"]:checked').val();
    	 ...
    	 $.ajax({
            type: "POST",
              url:"{{route('weekly.group.answer.submit')}}",
              data: {
              	'question_set_id': question_set_id,
            ...
            type: "POST",
              ...
              success: function(jsn) {
                ...
                if(jsn.status == 'true')
                {
                	$('#start-div').hide();
    				$('#exam-start').show();
    				$('#count').last().val(jsn.data.total_time);
                	$('#question_set_id').last().val(jsn.data.id);
                	$('#question').last().text(jsn.data.question);
                	$('#lableAnswer1').last().text(jsn.data.option_a);
                	$('#answer1').last().val(jsn.data.option_a);
    	 ...
    	 $.ajax({
            type: "POST",
              ...
              success: function(jsn) {
                ...
                else if(jsn.status == 'not-start')
                {
                	toastr["warning"](jsn.message);
                }
