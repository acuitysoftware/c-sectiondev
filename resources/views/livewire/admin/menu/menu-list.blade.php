<x-admin.table>
    ...
    <x-slot name="thead">
        <tr role="row">
            ...
            <th tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 30%;"
                aria-sort="ascending" aria-label="Agent: activate to sort column descending">Price
            </th>
        ...
        @forelse($menus as $menu)
            <tr role="row" class="odd">
                ...
                <td class="align-center"><span
                        class="kt-badge  kt-badge--{{ $menu->active == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $menu->id }})">{{ $menu->active == 1 ? 'Active' : 'Inactive' }}</span>
                </td>
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('menus.edit', ['menu' => $menu->id]) }}"><i
                            class="la la-edit"></i> Edit</a>
                    <button href="#" class="dropdown-item" wire:click="deleteAttempt({{ $menu->id }})"><i
