<x-admin.table>
        ...
        @forelse($boards as $board)
            <tr role="row" class="odd">
                ...
                <td class="align-center"><span
                        class="kt-badge  kt-badge--{{ $board->active == 1 ? 'success' : 'warning' }} kt-badge--inline kt-badge--pill cursor-pointer"
                        wire:click="changeStatusConfirm({{ $board->id }})">{{ $board->active == 1 ? 'Active' : 'Inactive' }}</span>
                </td>
                
                <x-admin.td-action>
                    <a class="dropdown-item" href="{{ route('boards.edit', ['board' => $board->id]) }}"><i
                            class="la la-edit"></i> Edit</a>
                    <button href="#" class="dropdown-item" wire:click="deleteAttempt({{ $board->id }})"><i
    ...
    <x-slot name="showingEntries">
        Showing {{ $boards->firstitem() }} to {{ $boards->lastitem() }} of {{ $boards->total() }} entries
    </x-slot>
</x-admin.table>
