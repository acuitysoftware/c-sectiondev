...
class QuestionSetList extends Component
{
    ...
    public $searchName, $searchRating, $searchStatus = -1, $perPage = 10, $chapter;
    ...
    {
        $questionSetQuery = QuestionSet::where('chapter_id',$this->chapter->id);
        if ($this->searchName)
            $questionSetQuery->where('question', 'like', '%' . $this->searchName . '%');
        
        if ($this->searchStatus >= 0)
            $questionSetQuery->where('active', $this->searchStatus);
        return view('livewire.admin.question-set.question-set-list', [
